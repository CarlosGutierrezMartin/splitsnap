...
export const SplitView: React.FC<SplitViewProps> = ({
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-4 pb-32 pt-5 animate-fade-in">
      <section className="mb-6 overflow-hidden rounded-hero bg-surface shadow-sm">
        <div className="border-b border-line bg-primary/5 p-6 text-center dark:bg-primary/10">
        ...
        <div className="p-5">
          ...
          <div
            className="h-3 overflow-hidden rounded-full bg-hair"
            role="progressbar"
      ...
      {receipt.participants.length === 0 && (
        <div className="mb-4 rounded-card border-2 border-dashed border-ghost p-6 text-center">
          <p className="font-semibold">{t.split.noPeople}</p>
          <p className="mt-1 text-sm text-muted">{t.split.noPeopleHint}</p>
        </div>
      )}
          ...
          {receipt.participants.map((participant) => (
            ...
            >
              ...
              <button
                ...
                className="rounded-lg p-2 text-faint transition-colors hover:bg-primary/10 hover:text-primary"
              >
      ...
      <div className="flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-control border-2 border-line bg-surface px-4 py-3 focus:border-primary focus:outline-none"
        />
      ...
      {receipt.participants.length > 1 && totals.remaining > 0.01 && (
        <button
          ...
          className="mt-4 flex w-full items-start gap-3 rounded-card border border-line bg-surface p-4 text-left transition-colors hover:border-primary/50"
        >
          ...
          <span>
            ...
            <span className="block text-xs text-muted">{t.split.splitRestHint}</span>
          </span>
        </button>
      )}

      {fullyAssigned && (
        <p className="mt-4 flex items-center justify-center gap-2 rounded-card bg-emerald-50 p-4 font-bold text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400">
